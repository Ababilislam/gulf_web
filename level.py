def level():
    ...
    if btn_search:
        ...
        search_level = db.executesql(search_level_sql,as_dict=True)[0]
    ...
    if depth == 0:
        ...
        if submit_btn:
            ...
            if national_id == '' or national_id == 'None' or national_id==None:
                ...
                insert_national_sql="INSERT INTO sm_level (cid,level_id,level_name,parent_level_id,parent_level_name,is_leaf,depth,level0,level0_name) VALUES ('"+str(c_id)+"','"+str(national_id)+"','"+str(national_name)+"','0','','0','0','"+str(national_id)+"','"+str(national_name)+"');"
                inset_national=db.executesql(insert_national_sql)
                response.flash='Successfully saved!'

        if update_btn:
            item_id = request.vars.item_id
            update_level_sql = "UPDATE sm_level SET level_name = '"+str(national_name)+"' , level0_name = '"+str(national_name)+"' WHERE id = '"+str(item_id)+"' ;"
        ...
        if submit_btn:
            if divisional_id == '' or divisional_id == 'None' or divisional_id==None:
                ...
                insert_nat_sql="INSERT INTO sm_level (cid,level_id,level_name,parent_level_id,parent_level_name,is_leaf,depth,level0,level0_name,level1,level1_name) VALUES ('"+str(c_id)+"','"+str(divisional_id)+"','"+str(divisional_name)+"','"+str(parent_id)+"','"+str(parent_name)+"','0','1','"+str(parent_id)+"','"+str(parent_name)+"','"+str(divisional_id)+"','"+str(divisional_name)+"');"
        ...
        if update_btn:
            item_id = request.vars.item_id
            update_level_sql = "UPDATE sm_level SET level_name = '"+str(divisional_name)+"',level1_name = '"+str(divisional_name)+"' WHERE id = '"+str(item_id)+"' ;"
            db.executesql(update_level_sql)
            response.flash='Update Successfully!'
        if delete_btn:
            item_id = request.vars.item_id
        ...
        if update_btn:
            item_id = request.vars.item_id
            distributor_name= request.vars.distributor_name
            update_level_sql = "UPDATE sm_level SET level_name = '"+str(distributor_name)+"',level2_name = '"+str(distributor_name)+"' WHERE id = '"+str(item_id)+"' ;"
