def rep():
    ...
    if submit:
        ...
        if rep_id=='' or rep_id=="None" or rep_id==None:
            ...
            if len(check_rep)>0:
                ...
                # return 12
                insert_rep_sql="INSERT INTO sm_rep (cid,rep_id,name,mobile_no,designation,password,status,user_type) VALUES ('"+str(c_id)+"','"+str(rep_id)+"','"+str(name)+"','"+str(mobile_no)+"','"+str(designation)+"','"+str(password)+"','"+str(status)+"','rep'); "
    ...
    limitby = (page * rep_per_page, (page + 1) * rep_per_page + 1)
    # return limitby
    # --------end paging
    ...
    repRows_sql = "select * from sm_rep where cid = '"+str(c_id)+"' "+condition+" and user_type='rep' ORDER BY id DESC  limit %d, %d;" % limitby
    repRows = db.executesql(repRows_sql, as_dict=True)
    session.condition = condition
    # return 22

    total_record_sql = f"SELECT COUNT(id) AS total FROM sm_rep WHERE cid='GULF' {condition} and user_type='rep' ORDER BY id ASC;"
    total_record = db.executesql(total_record_sql, as_dict = True)
    total_rec = total_record[0]['total']

    return dict(repRows=repRows,page=page,rep_per_page=rep_per_page,total_rec=total_rec)
...
def get_rep_id_list():
    ...
    replistRows_sql = "select rep_id,name from sm_rep where cid = '"+c_id+"' and user_type='rep' and status='ACTIVE';"
...
def get_sup_id_list():
    ...
    replistRows_sql = "select rep_id,name from sm_rep where cid = '"+c_id+"' and user_type='sup' and status='ACTIVE';"
    replistRows = db.executesql(replistRows_sql, as_dict=True)
            ...
            if len(coloum_list) != 6:
                ...
                if (repToId_excel==''or repToId_excel == 'NONE') and (name_excel=='' or name_excel== 'NONE') and (mobileNo_excel=='' or mobileNo_excel == 'NONE') and (designation_excel=='' or designation_excel == 'NONE') and (password_excel=='' or password_excel== 'NONE'):
                    ...
                    if len(existCheck) > 0:
                        ...
                        try:
                            insert_sql = "INSERT INTO sm_rep (cid,rep_id,name,mobile_no,designation,password,status,user_type) VALUES ('"+str(c_id)+"','"+str(repToId_excel)+"','"+str(name_excel)+"', '"+str(mobileNo_excel)+"','"+str(designation_excel)+"','"+str(password_excel)+"','"+str(status_excel)+"','rep');"
                            # return insert_sql
