def sup_area() :
	...
	if submit:
		...
		# return depth
		check_rep_sql = "select * from sm_rep where cid='"+str(c_id)+"'and rep_id='"+str(rep_id)+"' and user_type='sup';"
		# return check_rep_sql
		check_rep = db.executesql(check_rep_sql, as_dict=True)

		if len(check_rep)>0:

			chk_area_sql="select * from sm_level where cid='"+str(c_id)+"'and level_id='"+str(area_id)+"' and is_leaf='0' and (depth ='0' or depth='1');"
			# return chk_area_sql
			...
			if len (check_area)==0:
				...
				check_dup_sql = "select * from sm_supervisor_level where cid='"+str(c_id)+"'and sup_id='"+str(rep_id)+"' and level_id='"+str(area_id)+"';"
				# return check_rep_sql
				...
				if len(check_dup)>0:
					...
					insertArea_sql = "INSERT INTO  sm_supervisor_level (cid, sup_id , sup_name, level_id ,level_name,level_depth_no) VALUES ('"+str(c_id)+"','"+str(rep_id)+"','"+str(rep_name)+"','"+str(area_id)+"','"+str(area_name)+"','"+str(depth)+"');"      
					#return insertCrud_sql
					insertArea = db.executesql(insertArea_sql)
					response.flash= 'Inserted Successfully'
		else:
			response.flash = 'Supervisor is not valid!'
	...
	if btn_filter_rep == "Filter":
	    ...
	    if rep_id_filter_1 !='':
	    	...
	    	condition = condition + " and sup_id = '"+str(rep_id_filter_1)+"'"
	    	# return condition
	if btn_all == "All":
	    # return 22
	    condition = ''
	...
	if delete_btn: 
	    ...
	    # return area_Id
	    delete_sql = "DELETE from sm_supervisor_level where cid = '"+c_id+"' and sup_id='"+str(rep_Id)+"' and level_id='"+str(area_Id)+"'   limit 1 ;"
	    # return delete_sql
	    records = db.executesql(delete_sql)
	    session.flash = 'Deleted Successfully'
	    redirect (URL('sup_area_define','sup_area'))


	repArea_sql = "select * from sm_supervisor_level where cid = '"+str(c_id)+"' "+condition+" ;"
...
def sup_area_batch() :
	if (session.cid=='' or session.cid==None):
		redirect (URL('default','index'))
	response.title = 'sup-level Batch Upload'
	cid=session.cid
	...
	if btn_upload=='Upload':
		...
		for i in range(total_row):
			...
			if len(coloum_list)!=5:
				...
				depthExcel=  str(coloum_list[4]).strip().upper()             
	                
				if rep_idExcel==''or rep_idExcel == 'NONE' or nameExcel=='' or nameExcel== 'NONE' or area_idExcel=='' or area_idExcel == 'NONE' or area_nameExcel=='' or area_nameExcel == 'NONE' or depthExcel=='' or depthExcel=='None':
					...
					check_rep_sql = "select * from sm_rep where cid='"+str(cid)+"'and rep_id='"+str(rep_idExcel)+"' and user_type='sup' LIMIT 0,1; "
					# return check_rep_sql
					check_rep = db.executesql(check_rep_sql, as_dict=True)

					if len(check_rep)>0:

						chk_area_sql="select * from sm_level where cid='"+str(cid)+"'and level_id='"+str(area_idExcel)+"' and is_leaf='0' and (depth ='1' or depth ='0');"
						# return chk_area_sql
						check_area = db.executesql(chk_area_sql, as_dict=True)
						...
						if len (check_area)==0:
							...
							existCheckRows= " select * FROM sm_supervisor_level WHERE cid='"+str(cid)+"' and sup_id = '"+str(rep_idExcel)+"' and level_id= '"+str(area_idExcel)+"' LIMIT 0,1"
							...
							if len(existCheck) > 0:
								...
								try:
									insert_sql = "INSERT INTO sm_supervisor_level (cid, sup_id, sup_name, level_id, level_name,level_depth_no) VALUES ('"+str(cid)+"','"+str(rep_idExcel)+"','"+str(nameExcel)+"', '"+str(area_idExcel)+"','"+str(area_nameExcel)+"','"+str(depthExcel)+"');"
									# return insert_sql
...
def get_sup_area_id_list():
	if (session.cid=='' or session.cid==None):
		redirect (URL('default','index'))
	c_id = session.cid
	retStr = ''
	replistRows_sql = "select * from sm_level where cid = '"+c_id+"' and is_leaf='0';"
...
def area_list_Download():
	...
	repAreaRows_sql = "select * from sm_supervisor_level where cid = '"+c_id+"' "+condition+" ;"
	# return crudRows_sql
	rep_area = db.executesql(repAreaRows_sql, as_dict=True)
	# return record
	myString = 'Rep-Distributor List\n\n'
	myString += 'sup ID, sup Name, Area Id, Area Name, Depth\n'
	total=0
	attTime = ''
	totalCount = 0
	for i in range(len(rep_area)):
		recordsStr = rep_area[i]
		rep_id = str(recordsStr['sup_id'])
		rep_name = str(recordsStr['sup_name'])
		area_id = str(recordsStr['level_id'])
		area_name = str(recordsStr['level_name'])
		level_depth_no= str(recordsStr['level_depth_no'])

		myString += str(rep_id) + ',' + str(rep_name) + ',' + str(area_id) + ',' + str(area_name) + ',' + str(level_depth_no) + ',\n'


	# Save as csv
	import gluon.contenttype
	response.headers['Content-Type'] = gluon.contenttype.contenttype('.csv')
	response.headers['Content-disposition'] = 'attachment; filename=download_sup_area.csv'
	return str(myString)
...
def get_sup_id_list():
	...
	replistRows_sql = "select rep_id,name from sm_rep where cid = '"+c_id+"' and user_type='sup';"
	replistRows = db.executesql(replistRows_sql, as_dict=True)
